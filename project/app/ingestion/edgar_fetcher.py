...
class EdgarFetcher:
    ...
    def _make_request(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch from SEC EDGAR {url}: {str(e)}", exc_info=True
            )
            raise EdgarFetcherError(f"Failed to fetch from SEC EDGAR: {str(e)}") from e
        except json.JSONDecodeError as e:
            logger.error(
                f"Invalid JSON response from SEC EDGAR {url}: {str(e)}", exc_info=True
            )
            raise EdgarFetcherError(
                f"Invalid JSON response from SEC EDGAR: {str(e)}"
            ) from e
    ...
    def get_filing_history(self, cik: str) -> Dict[str, Any]:
        ...
        if len(cik) != 10 or not cik.isdigit():
            ...
            raise EdgarFetcherError(
                f"Invalid CIK format: {cik}. Must be 10-digit string."
            )
    ...
    def get_recent_filings(
        self, cik: str, form_types: Optional[List[str]] = None, max_filings: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to get recent filings for CIK {cik}: {str(e)}", exc_info=True
            )
            raise EdgarFetcherError(f"Failed to get recent filings: {str(e)}") from e

    def download_filing_text(
        self, cik: str, accession_number: str, form_type: str
    ) -> str:
        """
        ...
        try:
            ...
            index_url_with_dashes = (
                f"{base_url}/{cik}/{acc_path_with_dashes}/index.json"
            )
                            ...
                            if isinstance(directory, dict):
                                ...
                                for item in items:
                                    if isinstance(item, dict):
                                        ...
                                        if (
                                            form_type.replace("-", "").lower()
                                            in doc_name.lower()
                                            or doc_type == form_type
                                            or (
                                                doc_name
                                                and not doc_name.startswith("ex-")
                                            )
                                        ):
        ...
        for idx, ticker in enumerate(tickers, 1):
            try:
                ...
                filings = self.get_recent_filings(
                    cik, form_types=form_types, max_filings=max_filings_per_company
                )
                ...
                for filing_idx, filing in enumerate(filings, 1):
                    try:
                        ...
                        content = self.download_filing_text(
                            cik, filing["accession_number"], filing["form"]
                        )
                ...
                ticker_docs = len(
                    [d for d in documents if d.metadata.get("ticker") == ticker]
                )
    ...
    def save_filings_to_files(
        self, documents: List[Document], output_dir: Path
    ) -> List[Path]:
        """
