...
from app.ingestion import create_pipeline, IngestionPipelineError
from app.rag import create_rag_system, RAGQueryError
...
def verify_document_count(pipeline, min_count: int = 50, max_count: int = 100) -> bool:
    ...
    try:
        ...
        if doc_count < min_count:
            print(f"⚠ Warning: Document count ({doc_count}) is below minimum ({min_count})")
            ...
            print(f"✓ Document count ({doc_count}) meets requirement ({min_count}-{max_count})")
            return True
    ...
    for i, query in enumerate(test_queries, 1):
        try:
            ...
            if result['chunks_used'] > 0:
                ...
                print(f"  Note: This may be normal if query doesn't match document content")
                failed += 1
...
def verify_metadata_storage(pipeline) -> bool:
    ...
    try:
        ...
        if sample_results and len(sample_results['metadatas']) > 0:
            ...
            found_fields = [field for field in expected_fields if field in metadata_keys]
            
            if found_fields:
                print(f"✓ Expected metadata fields present: {', '.join(found_fields)}")
                return True
            else:
                print(f"⚠ Expected metadata fields not found (may vary by document type)")
    ...
    try:
        ...
        if sample_results := collection.peek(limit=1):
            if 'embeddings' in sample_results and sample_results['embeddings']:
                print(f"✓ Embeddings stored: {len(sample_results['embeddings'][0])} dimensions")
            else:
