...
def test_pipeline_process_document_error_handling(
    pipeline_comprehensive, test_documents_dir
):
...
@pytest.mark.integration
def test_pipeline_process_document_embedding_error_path(
    pipeline_comprehensive, test_documents_dir
):
    ...
    try:
        ...
        assert (
            "Embedding" in str(e) or "embedding" in str(e).lower()
...
def test_pipeline_process_document_chromadb_error_handling(
    ...
    try:
        ...
        try:
            ...
            assert (
                "ChromaDB" in str(e) or "storage" in str(e).lower()
            ), "Should indicate ChromaDB error"
    finally:
        if test_file.exists():
            test_file.unlink()
...
@pytest.mark.integration
def test_pipeline_process_documents_error_continues(
    pipeline_comprehensive, test_documents_dir
):
...
def test_pipeline_create_pipeline_factory(embedding_generator):
    ...
    assert isinstance(
        pipeline, IngestionPipeline
    ), "Should return IngestionPipeline instance"
    assert (
        pipeline.chroma_store.collection_name == collection_name
