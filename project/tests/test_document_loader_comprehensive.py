...
def test_chunk_document_method(loader, production_financial_document_1):
    ...
    assert all(chunk.metadata["type"] == "text" for chunk in chunks), \
        "All chunks should preserve type metadata"


@pytest.mark.unit
def test_process_documents_success(loader, test_documents_dir, production_financial_document_1):
...
@pytest.mark.unit
def test_process_documents_partial_failure(loader, test_documents_dir, sample_text_content):
...
def test_get_file_type_method(loader, test_documents_dir):
    ...
    try:
        assert loader._get_file_type(text_file) == "text", "Should return 'text' for .txt"
        assert loader._get_file_type(md_file) == "markdown", "Should return 'markdown' for .md"
    finally:
        if text_file.exists():
            text_file.unlink()
        if md_file.exists():
...
def test_load_markdown_file(loader, test_documents_dir, sample_markdown_content):
    ...
    try:
        ...
        assert "# Sample Markdown" in document.page_content or "Markdown" in document.page_content, \
