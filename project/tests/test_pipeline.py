...
def test_pipeline_process_single_document(
    ...
    assert all(isinstance(id_val, str) for id_val in chunk_ids), "All IDs should be strings"
    ...
    for doc_text in results["documents"][:2]:
        ...
        assert len(test_embedding) == embedding_generator.get_embedding_dimensions(), \
...
def test_pipeline_process_multiple_documents(
    ...
    assert final_count == initial_count + len(all_chunk_ids), \
    ...
    sample_ids = all_chunk_ids[:min(5, len(all_chunk_ids))]  # Sample from different documents
    results = production_pipeline.chroma_store.get_by_ids(sample_ids)
    assert len(results["ids"]) == len(sample_ids), "Should retrieve all requested chunks"
...
def test_pipeline_without_storage(production_pipeline, production_financial_file_1):
    ...
    assert all(id_val.startswith("chunk_") for id_val in chunk_ids), \
