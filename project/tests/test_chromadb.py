...
def test_add_documents(test_store, production_financial_documents, embedding_generator):
    ...
    assert len(embeddings) == len(
        production_financial_documents
    ), "Should generate embeddings for all documents"
    assert all(
        len(emb) == embedding_generator.get_embedding_dimensions() for emb in embeddings
...
def test_count(test_store, production_financial_documents, embedding_generator):
    ...
    assert count == len(
        production_financial_documents
    ), "Count should match number of documents added"


@pytest.mark.integration
@pytest.mark.chromadb
def test_query_by_embedding(
    test_store, production_financial_documents, embedding_generator
):
    ...
    assert (
        len(query_embedding) == embedding_generator.get_embedding_dimensions()
...
def test_get_by_ids(test_store, production_financial_documents, embedding_generator):
    ...
    if results["documents"]:
        assert (
            results["documents"][0] == production_financial_documents[0].page_content
...
def test_metadata_filtering(
    ...
    if results["ids"] and "metadatas" in results:
        for metadata in results["metadatas"]:
            if metadata:
                assert (
                    metadata.get("type") == "10-K"
                ), "Results should match type filter"
                assert (
                    metadata.get("company") == "test_company"
                ), "Results should match company filter"
