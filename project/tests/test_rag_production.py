...
def test_rag_system_initialization(production_rag_system):
    ...
    assert (
        production_rag_system.chroma_store is not None
    ), "ChromaDB store should be initialized"
    assert (
        production_rag_system.embedding_generator is not None
    ), "Embedding generator should be initialized"
...
def test_rag_query_with_revenue_document(populated_rag_system):
    ...
    assert any(
        "revenue" in str(source).lower() or "10-K" in str(source)
...
def test_rag_query_with_risk_document(populated_rag_system):
    ...
    assert any(
        keyword in answer_lower for keyword in ["risk", "market", "regulation", "legal"]
...
def test_rag_query_with_liquidity_document(populated_rag_system):
    ...
    assert any(
        keyword in answer_lower for keyword in ["cash", "110.5", "billion", "operating"]
...
def test_rag_query_simple_interface(populated_rag_system):
    ...
    assert (
        ...
        or "394.3" in answer.lower()
        or "billion" in answer.lower()
...
def test_rag_query_multiple_queries(populated_rag_system):
    ...
    for query in queries:
        ...
        ), f"Query '{query}' should return substantial answer"

    # Verify all queries returned different answers
    answers = [r["answer"] for r in results]
    assert len(set(answers)) == len(
        answers
    ), "Different queries should return different answers"
